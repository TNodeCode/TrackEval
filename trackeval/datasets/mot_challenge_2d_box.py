...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self.class_name_to_class_id = {v: k+1 for k, v in enumerate(self.config['CLASSES_TO_EVAL'])}
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        distractor_class_names = self.config['DISTRACTOR_CLASSES']
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            to_remove_tracker = np.array([], np.int8)
        ...
        if len(unique_gt_ids) > 0:
            ...
            for t in range(raw_data['num_timesteps']):
                if len(data['gt_ids'][t]) > 0:
                    data['gt_ids'][t] = gt_id_map[data['gt_ids'][t]].astype(np.int8)
        if len(unique_tracker_ids) > 0:
            ...
            for t in range(raw_data['num_timesteps']):
                if len(data['tracker_ids'][t]) > 0:
                    data['tracker_ids'][t] = tracker_id_map[data['tracker_ids'][t]].astype(np.int8)
