...
class HOTA(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        if data['num_tracker_dets'] == 0:
            res['HOTA_FN'] = data['num_gt_dets'] * np.ones((len(self.array_labels)), dtype=np.float8)
            res['LocA'] = np.ones((len(self.array_labels)), dtype=np.float8)
            res['LocA(0)'] = 1.0
            return res
        if data['num_gt_dets'] == 0:
            res['HOTA_FP'] = data['num_tracker_dets'] * np.ones((len(self.array_labels)), dtype=np.float8)
            res['LocA'] = np.ones((len(self.array_labels)), dtype=np.float8)
            res['LocA(0)'] = 1.0
            return res
