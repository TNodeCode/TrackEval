...
class TrackMAP(_BaseMetric):
    ...
    def combine_sequences(self, all_res):
        ...
        for ig_idx in range(self.num_ig_masks):
            ...
            tp_sum = np.cumsum(tps, axis=1).astype(dtype=np.float8)
            fp_sum = np.cumsum(fps, axis=1).astype(dtype=np.float8)
        ...
        for lbl in self.lbls:
            res['AP_' + lbl] = np.zeros((len(self.array_labels)), dtype=np.float8)
            res['AR_' + lbl] = np.zeros((len(self.array_labels)), dtype=np.float8)
    ...
    def combine_classes_class_averaged(self, all_res, ignore_empty_classes=True):
        ...
        res = {}
        for field in self.fields:
            res[field] = np.zeros((len(self.array_labels)), dtype=np.float8)
